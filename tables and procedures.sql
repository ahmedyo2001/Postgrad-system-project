...
select @sum = p.amount from payment p where p.id = @paymentID
declare @instnum int 
select @instnum= p.noOfInstallments from payment p where p.id = @paymentID
declare @perinst int
set @perinst = @sum / @instnum
insert into installment (date , paymentId, amount, done) values (@InstallStartDate , @paymentID, @perinst,  0);
set @instnum = @instnum -1
...
@ThesisSerialNo int , @DefenseDate Datetime , @ExaminerID int
as
...
@title varchar(50), @pubDate datetime, @host varchar(50), @place varchar(50), @accepted bit,@id int output
...
select @temp=  max(id) from publication 
set @id=@temp
go
...
go
CREATE Proc ViewExaminerResponsibilities
@Examinerid int
As
select e.name,sup.name,T.title,GS.firstName,Gs.lastName,ngs.firstName,ngs.lastName
from ExaminerEvaluateDefense ee inner join examiner e on ee.examinerId=e.id
inner join GUCianStudentRegisterThesis s on ee.serialNo=s.serial_no
inner join Supervisor sup on sup.id=s.supid inner join Thesis T on T.serialNumber=s.serial_no
inner join GucianStudent GS on GS.id=s.sid inner join NonGUCianStudentRegisterThesis ns on ee.serialNo=ns.serial_no
inner join NonGucianStudent NGS on Ngs.id=ns.sid
where @Examinerid=e.id
...
@examinerid int
as
select T.* from Thesis T inner join ExaminerEvaluateDefense ee on T.serialNumber=EE.serialNo 
where ee.examinerId=@examinerid and T.title like '%'+@title+'%'

go

create Proc viewAllSup
As
select * from Supervisor

go;
create proc showThesis
@studentID int
as
if (exists(select * from GucianStudent where id=@studentID))
select e1.* from thesis e1 inner join GUCianStudentRegisterThesis g1 on e1.serialNumber=g1.serial_no
where g1.sid=@studentID

else

select e1.* from thesis e1 inner join NonGUCianStudentRegisterThesis g1 on e1.serialNumber=g1.serial_no
where g1.sid=@studentID
...
as
select C1.*,G1.grade
from Course C1 inner join NonGucianStudentTakeCourse G1 on G1.cid=C1.id
where G1.sid = @studentID
...
@Examinerid int
As
select e.name as Examiner_Name,sup.name as Supervisor_Name,T.title as Thesis_Title,GS.firstName ,Gs.lastName,ngs.firstName,ngs.lastName
from ExaminerEvaluateDefense ee inner join examiner e on ee.examinerId=e.id
inner join GUCianStudentRegisterThesis s on ee.serialNo=s.serial_no
inner join Supervisor sup on sup.id=s.supid inner join Thesis T on T.serialNumber=s.serial_no
inner join GucianStudent GS on GS.id=s.sid inner join NonGUCianStudentRegisterThesis ns on ee.serialNo=ns.serial_no
inner join NonGucianStudent NGS on Ngs.id=ns.sid
where @Examinerid=ee.examinerId
...
as
select T.* from Thesis T inner join ExaminerEvaluateDefense ee on T.serialNumber=EE.serialNo 
where ee.examinerId=@id and T.title like '%'+@title+'%'
